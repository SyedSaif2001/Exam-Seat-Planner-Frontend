  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        // 4. Find invigilator duties
        const duties = (allPlans || []).filter(plan =>
          plan.invigilator_email === profileData.email
        );
        setInvigilatorDuties(duties);
        setLoadingDuties(false);

        // 5. Fetch all exams for extra info
        ...
        {loadingPlans ? (
          ...
          <Container className="py-10">
            ...
            {seatingPlans.length === 0 ? (
              <div>No seating plans available.</div>
            ) : (
              seatingPlans.map((plan, i) => {
                const rooms = plan.rooms || [];
                const exam = exams.find(e => (e._id || e.ID)?.toString() === (plan.exam_id || plan.ExamID)?.toString());
                const examTitle = exam?.title || plan.exam_title || "Exam";
                ...
                const planDateStr = planDate ? new Date(planDate).toLocaleDateString() : "";
                  ...
                  <div key={i} className="flex justify-between items-center py-4 border-b last:border-b-0">
                    <div>
                      ...
                      <p className="text-gray-500">
                        ...
                        {planDateStr && <span>Created: {planDateStr}</span>}
                      </p>
                    </div>
                    <div className="flex gap-4">
                      <button onClick={() => handleViewDetails(plan)} className="text-blue-600 hover:underline">
                        View Details
                      </button>
