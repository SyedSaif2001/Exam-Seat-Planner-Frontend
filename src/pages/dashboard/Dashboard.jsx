...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      ...
      try {
        const [examsRes, studentsRes, roomsRes, plansRes, listsRes] =
          await Promise.all([
        ...
        const [examsData, studentsData, roomsData, plansData, listsData] =
          await Promise.all([
            examsRes.json(),
            studentsRes.json(),
            roomsRes.json(),
      ...
      (planData.rooms || []).forEach((room) => {
        (room.student_lists || []).forEach((list) => {
          (list.students || []).forEach((s) => {
            ...
            if (studentWithDept.student_id)
              studentMap[String(studentWithDept.student_id)] = studentWithDept;
            if (studentWithDept.StudentID)
              studentMap[String(studentWithDept.StudentID)] = studentWithDept;
            if (studentWithDept.cms_id)
              studentMap[String(studentWithDept.cms_id)] = studentWithDept;
          });
        });
      });
  ...
  const handleDeletePlan = async (plan) => {
    ...
    try {
      ...
      setSeatingPlans((prev) => prev.filter((p) => (p._id || p.ID) !== planId));
    } catch (err) {
              ...
              {seatingPlans.length === 0 ? (
                ...
                seatingPlans.map((plan, idx) => {
                  const exam = exams.find(
                    (e) =>
                      (e._id || e.ID)?.toString() ===
                      (plan.exam_id || plan.ExamID)?.toString()
                  );
                  const examTitle =
