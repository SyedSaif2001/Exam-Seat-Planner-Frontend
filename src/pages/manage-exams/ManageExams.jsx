              ...
              return (
                ...
                >
                  ...
                  <span className="text-xs text-gray-500">
                    ({list.total_count} students)
                  </span>
                  {isSelected && (
                    <CheckCircle className="inline ml-2 text-green-500 w-4 h-4" />
                  )}
                </div>
              );
            })
              ...
              return (
                ...
                >
                  <input
                    ...
                    disabled={isAssigned}
                    checked={
                      Array.isArray(selected) && selected.includes(inv._id)
                    }
                    onChange={() => !isAssigned && onSelect(inv._id)}
                  />
                  <span className="font-medium">{inv.name}</span>{" "}
                  <span className="text-xs text-gray-500">({inv.email})</span>{" "}
...
const EditExamModal = ({ isOpen, onClose, exam, onSave }) => {
  ...
  const [invigilatorModal, setInvigilatorModal] = useState({
    open: false,
    roomIndex: null,
  });
  ...
  const handleAddRoom = () => {
    ...
    const room = allRooms.find(
      (r) => r._id === selectedRoomId || r.ID === selectedRoomId
    );
    if (!room) return;
  ...
  const fetchInvigilators = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        "http://localhost:8080/api/seating/invigilators",
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const handleGenerateSeatingPlan = async () => {
    ...
    try {
      ...
      if (
        rooms.length > 0 &&
        rooms[0].invigilatorIds &&
        rooms[0].invigilatorIds.length > 0
      ) {
        const firstInvigilator = invigilators.find(
          (inv) => inv._id === rooms[0].invigilatorIds[0]
        );
      ...
      if (!invigilatorEmail) {
        setError(
          "Please assign at least one invigilator to generate seating plans"
        );
        setLoading(false);
        return;
      }
      ...
      const response = await fetch(
        "http://localhost:8080/api/seating/generate",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const updateExamWithRooms = async () => {
    ...
    try {
      ...
      const examResponse = await fetch(
        `http://localhost:8080/api/seating/exams/${examId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const getStudentListNames = (studentListIds) => {
    if (!Array.isArray(studentListIds) || studentListIds.length === 0)
      return "Not selected";
  ...
  const getInvigilatorNames = (invigilatorIds) => {
    if (!Array.isArray(invigilatorIds) || invigilatorIds.length === 0)
      return "";
  ...
  const handleInvigilatorSelect = (roomIndex, invigilatorId) => {
    ...
    const isAlreadyAssigned = rooms.some(
      (room, idx) =>
        idx !== roomIndex &&
        room.invigilatorIds &&
    ...
    if (isAlreadyAssigned) {
      ...
      setError(
        `Invigilator "${invigilatorName}" is already assigned to another room in this exam. Each invigilator can only be assigned to one room per exam.`
      );
      return;
    }
    if (updatedRooms[roomIndex].invigilatorIds.includes(invigilatorId)) {
      updatedRooms[roomIndex].invigilatorIds = updatedRooms[
        roomIndex
      ].invigilatorIds.filter((id) => id !== invigilatorId);
    } else {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
      <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="simple">
                    Simple Fill (Row-wise, No Constraints)
                  </option>
                  <option value="separated">No Adjacent Same Department</option>
                </select>
                <p className="text-xs text-gray-500 mt-1">
                  Parallel: Students arranged by columns | Simple Fill:
                  Row-wise, no constraints | No Adjacent Same Department: Leaves
                  seats empty if only adjacent-department students are available
                </p>
              </div>
            </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-xs text-gray-500 mb-1">
                          Rows: {room.rows}, Cols: {room.columns}, Cap:{" "}
                          {room.capacity}
                        </div>
                        ...
                        <tr key={index}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            <button
                              ...
                              className="text-blue-600 hover:text-blue-800"
                            >
                              {getInvigilatorNames(room.invigilatorIds) ||
                                "Add Invigilator(s)"}
                            </button>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
        ...
        <StudentListModal
          ...
          onSelect={(id) =>
            handleStudentListSelect(studentListModal.roomIndex, id)
          }
        ...
        <InvigilatorModal
          ...
          onSelect={(id) =>
            handleInvigilatorSelect(invigilatorModal.roomIndex, id)
          }
          selected={rooms[invigilatorModal.roomIndex]?.invigilatorIds || []}
          assignedInvigilators={rooms.flatMap((room, idx) =>
            idx !== invigilatorModal.roomIndex ? room.invigilatorIds || [] : []
          )}
        />
      </div>
    </div>
...
const ManageExams = () => {
      ...
      try {
        ...
        const sorted = [...data].sort((a, b) => {
          const dateA = new Date(
            a.created_at || a.CreatedAt || a.Date || a.date || a._id || a.ID
          );
          const dateB = new Date(
            b.created_at || b.CreatedAt || b.Date || b.date || b._id || b.ID
          );
          return dateB - dateA;
        });
        setExams(sorted);
  ...
  const handleEditSave = (updatedExam) => {
    setExams((prev) =>
      prev.map((exam) =>
        exam.ID === updatedExam.ID ? { ...exam, ...updatedExam } : exam
      )
    );
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row) => {
        ...
        return row.Duration !== undefined && row.Duration !== null
          ? row.Duration
          : "-";
      },
    },
